...
$bigram_incr = $dbh->prepare("UPDATE `bigram` SET `count` = `count`+1 WHERE `bigram`=:bigram");
$bigram_concat = $dbh->prepare("UPDATE `bigram` SET `key_id`=CONCAT(`key_id`,:id) WHERE `bigram`=:bigram");
...
for ($i=$from; $i<$to; $i++){
    ...
    foreach ($bigram_arr as $value){
        ...
        if(count($count)==0){
            ...
            $bigram_incr->bindParam(":bigram", $value);
            $bigram_incr->execute();

            $id_str = ", ".$id;
            $bigram_concat->bindParam(":id", $id_str);
            $bigram_concat->bindParam(":bigram", $value);
