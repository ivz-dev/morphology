...
$select = $dbh->prepare("SELECT * FROM `allkeys` WHERE id>=:from and id<:to"); //выборка ключей
$count_qw = $dbh->prepare("SELECT * FROM `new_trigram` WHERE trigram=:trigram_val"); //выборка для проверки наличия би-грамы
$trigram_insert = $dbh->prepare("INSERT INTO `new_trigram` (trigram, key_id, count) VALUES (:trigram, :key_id, :count)");
$trigram_incr = $dbh->prepare("UPDATE `new_trigram` SET `count` = `count`+1 WHERE `trigram`=:trigram");
$trigram_concat = $dbh->prepare("UPDATE `new_trigram` SET `key_id`=CONCAT(`key_id`,:id) WHERE `trigram`=:trigram");
